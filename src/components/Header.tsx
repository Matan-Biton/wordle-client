...
export function Header() {
  const { userName, handleHelpModal } = useContext(wordleContext);
  ...
  return (
    <header className="bg-teal-500 w-screen min-h-[10vh] flex items-center justify-between px-6 py-2">
      ...
      <div>
        <button className="bg-gray-400 border-2 border-gray-900 rounded-md px-4 hover:bg-gray-500" onClick={() => handleHelpModal(true)}>{userName === 'Guest' ? "Login" : "Change user"}</button>
